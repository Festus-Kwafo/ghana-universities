...
def store_in_db():
    ...
    # Create a table
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS data (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), ranking VARCHAR(255), website VARCHAR(255), logo VARCHAR(255), description TEXT, acronym VARCHAR(255), founded VARCHAR(255), motto VARCHAR(255), colors VARCHAR(255), address VARCHAR(255), tel VARCHAR(255), fax VARCHAR(255))")
    for i in range(len(data)):
        ...
        if result:
            sql_update = "UPDATE data SET ranking = %s, website = %s, logo = %s,description = %s,acronym = %s,founded = %s,motto = %s,colors = %s,address = %s,tel = %s,fax = %s  WHERE name = %s"
            ...
            sql_insert = "INSERT INTO data (name, ranking, website, logo, description, acronym, founded, motto, colors, address, tel, fax) VALUES (%s, %s, %s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
